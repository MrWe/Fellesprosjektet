...
public class ListController {
	...
	@FXML
	private void initialize() { // can't use this method since it is called
								// before mainApp is set
	}

	public void init2() {
		editGroupBtn.setDisable(true); // dårlig løsning til editBtn
		db = new DBConnection();
		ResultSet AllGroupsRS = db.getAllGroupsOfUser(mainApp.getUser()
				.getUsername());
		Map<String, TreeItem<Group>> groups = new HashMap<String, TreeItem<Group>>(); // HashMap
																						// that
																						// contains
																						// the
																						// usergroupID
																						...
																						// group
		try {
			while (AllGroupsRS.next()) { // fills the HashMap with all the
											// groups from the database
				ResultSet groupMembersRS = db.getGroupMembers(AllGroupsRS
						.getString(1)); // gets all groupmembers of the current
										// group
...
<<<<<<< HEAD
				...
				// " " + AllGroupsRS.getString(4));
				TreeItem<Group> treeItem = new TreeItem<Group>(new Group(
						AllGroupsRS.getString(3), AllGroupsRS.getString(2)
								.equals("1") ? true : false,
...
>>>>>>> origin/master
			for (int i = 0; i <= higheshGroupID; i++) {
				for (int j = 0; j <= higheshGroupID; j++) {
					...
					// usergroupID of group i (if group j is a subgroup of group
					// i):
					if (groups.containsKey("" + i)
							&& groups.containsKey("" + j)
							&& groups.get("" + j).getValue().getSupergroupID() != null
							&& groups.get("" + j).getValue().getSupergroupID()
									.equals("" + i)) {
						// add the TreeItem of group j to the children of the
						// TreeItem of grup i
						groups.get("" + i).getChildren()
								.add(groups.get("" + j));
		...
		treeView.getSelectionModel().selectedItemProperty()
				.addListener(new ChangeListener<TreeItem<Group>>() {
					@Override
					public void changed(
							ObservableValue<? extends TreeItem<Group>> observableValue,
							TreeItem<Group> oldValue, TreeItem<Group> newValue) {
						System.out.println("selection listener " + oldValue
								+ " " + newValue);
						if (!treeView.getRoot().getChildren()
								.contains(oldValue)) {
						...
						System.out.println("selectedItemProperty listener: "
						...
						try {
							if (!newValue.getValue().equals(
									treeView.getRoot().getValue())) {
								mainApp.showCalendar(newValue.getValue());
							} else {
								System.out
										.println("root group has been declined selection");
							}
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
						// treeView.setMinHeight(treeView.getExpandedItemCount()*37
						// + 2);
		...
		treeView.getRoot().getChildren()
				.addListener(new ListChangeListener<TreeItem<Group>>() {

					@Override
					public void onChanged(
							ListChangeListener.Change<? extends TreeItem<Group>> c) {
						c.next();
		...
		// treeView.getSelectionModel().getSelectedItem().expandedProperty().addListener(new
		// ChangeListener<Boolean>() {
		// @Override
	...
	private void editGroup() {
		mainApp.showEditGroupPopup(treeView, treeView.getSelectionModel()
				.getSelectedItem());
	}

	@FXML
	private void newSubGroup() {
		mainApp.showGroupPopup(treeView, treeView.getSelectionModel()
